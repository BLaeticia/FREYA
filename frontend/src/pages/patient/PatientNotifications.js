...
const typeConfig = {
  registration:       { icon: '🎉', bg: '#CCFBF1', label: 'Inscription' },
  new_appointment:    { icon: '📅', bg: '#DBEAFE', label: 'Rendez-vous' },
  appointment_update: { icon: '🔄', bg: '#FEF3C7', label: 'Mise à jour RDV' },
  new_message:        { icon: '💬', bg: '#F3E8FF', label: 'Message' },
  new_record:         { icon: '📄', bg: '#ECFDF5', label: 'Document' },
  reminder:           { icon: '⏰', bg: '#FEF3C7', label: 'Rappel' },
  admin_decision:     { icon: '⚙️', bg: '#F1F5F9', label: 'Administratif' },
};
    ...
    <div style={s.root} onClick={() => setShowUserMenu(false)}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        .nav-lnk:hover { background-color: #F8FAFC !important; }
        .notif-item:hover { background-color: #F8FAFC !important; cursor: pointer; }
      ...
      <main style={s.main}>
        ...
        {/* HEADER PAGE */}
        <div style={s.pageHeader} className="fade-in">
          <div>
        ...
        <div style={s.layout}>
          ...
          <div style={s.sidebar} className="fade-in">
            <div style={s.sideCard}>
              <div style={s.sideTitle}>Filtrer par</div>
              {[
                { id: 'all',                label: 'Toutes',          icon: '🔔' },
                { id: 'unread',             label: 'Non lues',        icon: '●' },
                { id: 'new_appointment',    label: 'Rendez-vous',     icon: '📅' },
                { id: 'new_message',        label: 'Messages',        icon: '💬' },
                { id: 'registration',       label: 'Inscription',     icon: '🎉' },
                { id: 'reminder',           label: 'Rappels',         icon: '⏰' },
                { id: 'new_record',         label: 'Documents',       icon: '📄' },
              ].map(f => (
                <button
                  key={f.id}
          ...
          <div style={s.content}>
            {loading ? (
              ...
              <div style={s.notifCard} className="fade-in">
                {filtered.map((n, i) => {
                  const cfg = typeConfig[n.type] || { icon: '🔔', bg: '#F1F5F9', label: 'Notification' };
                  return (
                    <div
                      key={n.id || i}
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '4px' }}>
                          <div style={{ fontSize: '13px', fontWeight: n.isRead ? '500' : '700', color: '#0F172A' }}>
                            {n.title}
                          </div>
                          <div style={{ fontSize: '11px', color: '#94A3B8', flexShrink: 0, marginLeft: '12px' }}>
                            {formatDate(n.createdAt)}
                          </div>
                        </div>
                        {n.body && (
                          <div style={{ fontSize: '12px', color: '#64748B', lineHeight: '1.5' }}>{n.body}</div>
                        )}
...
const s = {
  ...
  ctaBtn: { backgroundColor: '#0D9488', color: '#fff', padding: '9px 18px', borderRadius: '10px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', border: 'none' },
  ...
  pageTitle: { fontSize: '26px', fontWeight: '800', color: '#0F172A', marginBottom: '4px' },
  pageSubtitle: { fontSize: '14px', color: '#64748B' },
  markAllBtn: { backgroundColor: '#fff', border: '1.5px solid #0D9488', color: '#0D9488', padding: '10px 18px', borderRadius: '10px', fontSize: '13px', fontWeight: '600', cursor: 'pointer' },

  layout: { display: 'grid', gridTemplateColumns: '220px 1fr', gap: '20px', alignItems: 'start' },

  sidebar: { position: 'sticky', top: '85px' },
  sideCard: { backgroundColor: '#fff', borderRadius: '16px', border: '1px solid #E2E8F0', padding: '16px', display: 'flex', flexDirection: 'column', gap: '4px' },
  sideTitle: { fontSize: '11px', fontWeight: '700', color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '8px', padding: '0 8px' },
  filterBtn: (active) => ({
    ...
    color: active ? '#0D9488' : '#64748B', fontSize: '13px', fontWeight: active ? '700' : '500',
    cursor: 'pointer', textAlign: 'left', width: '100%',
  }),
  filterBadge: { marginLeft: 'auto', backgroundColor: '#0D9488', color: '#fff', fontSize: '10px', fontWeight: '700', padding: '1px 6px', borderRadius: '20px' },

  content: {},
  notifCard: { backgroundColor: '#fff', borderRadius: '16px', border: '1px solid #E2E8F0', overflow: 'hidden' },
  notifItem: (read) => ({
  ...
  unreadDot: { width: '8px', height: '8px', borderRadius: '50%', background: '#0D9488', flexShrink: 0, marginTop: '6px' },

  emptyState: { backgroundColor: '#fff', borderRadius: '16px', border: '1px solid #E2E8F0', padding: '60px 20px', textAlign: 'center' },
};