...
const s = {
  ...
  navbar:      { height: '64px', backgroundColor: '#fff', borderBottom: '1px solid #E2E8F0', display: 'flex', alignItems: 'center', padding: '0 40px', position: 'sticky', top: 0, zIndex: 10 },
  ...
  hero:        { background: 'linear-gradient(160deg, #1D4ED8 0%, #2563EB 45%, #60A5FA 100%)', padding: '60px 40px', position: 'relative', overflow: 'hidden', color: '#fff' },
  ...
  heroTitle:   { fontSize: '38px', fontWeight: '800', marginBottom: '12px', letterSpacing: '-1px' },
  ...
  container:   { maxWidth: '1100px', margin: '30px auto', padding: '0 20px', display: 'grid', gridTemplateColumns: '1fr 360px', gap: '20px', alignItems: 'start' },
        ...
        <div>
          <div style={s.rdvCard}>
            <div style={s.rdvHead}>
              ...
              <div style={s.rdvTitle}>Analyses médicales</div>
            </div>
            <div style={s.rdvBody}>
              {!success ? (
                <>
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: '#94A3B8', textTransform: 'uppercase', marginBottom: '6px' }}>Date souhaitée</label>
                  <input
                    type="date"
                    min={today}
                    value={rdvDate}
                    ...
                    style={s.rdvInput}
                  />
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: '#94A3B8', textTransform: 'uppercase', marginBottom: '6px' }}>Heure</label>
                  {!rdvDate ? (
                    <div style={{ ...s.rdvInput, color: '#94A3B8', backgroundColor: '#F8FAFC', display: 'flex', alignItems: 'center' }}>
                      Choisissez une date d'abord
                    </div>
                  ) : availableSlots.length === 0 ? (
                    <div style={{ ...s.rdvInput, color: '#EF4444', backgroundColor: '#FEF2F2', display: 'flex', alignItems: 'center' }}>
                      {new Date(rdvDate).getDay() === 5 ? 'Fermé le vendredi' : 'Aucun créneau disponible'}
                    </div>
                  ) : (
                    <select
                      value={rdvTime || availableSlots[0]}
                  ...
                  <label style={{ display: 'block', fontSize: '11px', fontWeight: '700', color: '#94A3B8', textTransform: 'uppercase', marginBottom: '6px' }}>Type d'analyse (optionnel)</label>
                  <input
                    type="text"
                  ...
                  <button
                    onClick={confirmRdv}
                    disabled={booking || !rdvDate || availableSlots.length === 0}
                    style={{ ...s.rdvBtn, opacity: (booking || !rdvDate || availableSlots.length === 0) ? 0.6 : 1 }}
                  >
                    {booking ? 'Réservation...' : isAuthenticated ? 'Confirmer le rendez-vous' : 'Connexion requise'}
                  </button>
                  {!isAuthenticated && (
                ...
                <div style={{ textAlign: 'center', padding: '20px 0' }}>
                  ...
                  <div style={{ fontWeight: '800', color: '#059669', fontSize: '16px', marginBottom: '6px' }}>Rendez-vous confirmé !</div>
                  <div style={{ fontSize: '13px', color: '#475569' }}>Le laboratoire a été notifié. Vos résultats seront disponibles dans votre dossier médical.</div>
                </div>
              )}
            </div>
          </div>
