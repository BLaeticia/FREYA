...
const NAV_LINKS = [
  { id: 'dashboard',    label: 'Tableau de bord', path: '/labo' },
  { id: 'appointments', label: 'Rendez-vous',     path: '/labo/appointments' },
  { id: 'analyses',     label: 'Analyses',         path: '/labo/analyses' },
  { id: 'messages',     label: 'Messages',          path: '/labo/messages' },
  { id: 'profile',      label: 'Profil',            path: '/labo/profile' },
];
...
export default function LabNavbar({ active }) {
  const { user, logout } = useAuthStore();
  const navigate = useNavigate();
  const [open, setOpen] = useState(false);

  const name    = user?.firstName || 'Laboratoire';
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white border-b border-slate-200 shadow-sm">
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center gap-4">
        <Link to="/labo" className="text-2xl font-extrabold text-primary-600 tracking-tight shrink-0 no-underline">
          Freya
        </Link>
        <span className="text-xs font-semibold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md shrink-0">Laboratoire</span>
        ...
        <div className="relative shrink-0" onClick={e => e.stopPropagation()}>
          ...
          >
            ...
            <div className="text-left">
              <div className="text-xs font-bold text-slate-900 leading-tight max-w-[120px] truncate">{name}</div>
              <div className="text-[10px] text-slate-400">Laboratoire</div>
            </div>
          </button>
